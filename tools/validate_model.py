...
class ModelValidator:
    ...
    def validate_on_dataset(
        ...
        Args:
            ...
            targets: Array of target stationarity scores (N,)
    ...
    def _compute_metrics(
        ...
        metrics.update(
            {
                "inference_time_min_ms": float(inference_times.min()),
                "inference_time_max_ms": float(inference_times.max()),
                "inference_time_mean_ms": float(inference_times.mean()),
                "inference_time_std_ms": float(inference_times.std()),
            }
        )

        return metrics

    def benchmark_inference(self, signal_length: int = 1000, n_runs: int = 100):
        """
...
def main():
    parser = argparse.ArgumentParser(
        description="Validate trained LSTM boundary prediction model"
    )
    parser.add_argument(
        "--model", type=str, required=True, help="Path to ONNX model file"
    )
    parser.add_argument(
        "--data", type=str, help="Path to training dataset (NPZ format)"
    )
    parser.add_argument(
        "--output",
        type=str,
    ...
    try:
        ...
        report = {
            ...
            "model_size_bytes": args.model_stat_size
            if hasattr(args, "model_stat_size")
            else Path(args.model).stat().st_size,
        }
