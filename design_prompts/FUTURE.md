# Future Work: Ferromode v2.x and Beyond

This document outlines planned features and improvements for Ferromode beyond the v1.x release scope (M0-M9). These items are explicitly deferred to future versions as noted in PRD.md and IMPLEMENTATION_MAP.md.
...
- Online/Streaming EMD: Process data in chunks as it arrives, maintaining state between chunks
- Sliding Window EMD: Fixed-window processing with overlap for continuous monitoring
- Adaptive Buffering: Dynamically adjust buffer size based on signal stationarity
- Boundary Prediction: Use AR models or neural networks to predict boundary conditions for streaming data
- Incremental IMF Extraction: Extract IMFs as they become stable rather than waiting for full decomposition
...
- Maintaining envelope continuity across chunk boundaries
- Adapting stopping criteria for partial signals
- Managing memory footprint in long-running processes
- Ensuring numerical equivalence with batch processing

### 2.1: GPU Acceleration
**Motivation:** Leverage GPU parallelism for large-scale EMD processing on massive datasets.
- CUDA Kernels: Parallel extrema detection, envelope interpolation, and sifting iterations
...
**Target Algorithms:**
- EEMD/CEEMDAN (embarrassingly parallel across trials)
- VMD (frequency-domain parallelism)
- MEMD (direction-wise parallelism)
...
2. Start with "good first issue" labeled issues in the GitHub repository
3. Follow the test-first development workflow
4. Engage with the maintainers through GitHub Discussions
5. Consider implementing reference implementations in Rust first before language bindings

---
*This document is a living artifact and will be updated as the project evolves. Last updated: 2026-04-04*
